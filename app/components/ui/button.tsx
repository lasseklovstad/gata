...
const buttonVariants = cva(
   "inline-flex items-center justify-center whitespace-nowrap uppercase rounded-md text font-medium ring-offset-background transition-colors focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
   {
      variants: {
         variant: {
...
export const ButtonResponsive = ({ label, icon, ...buttonProps }: ButtonResponsiveProps) => {
   return (
      <>
         <Button className="hidden md:flex gap-2 shrink-0" {...buttonProps}>
            {icon}
            {label}
         </Button>
         <Button size="icon" className="md:hidden  shrink-0" {...buttonProps}>
            {icon}
