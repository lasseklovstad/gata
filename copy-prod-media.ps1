...
$destinationFolder = Join-Path -Path "data" -ChildPath (Get-Date -Format "dd_MM_yyyy")
if (!(Test-Path $destinationFolder)) {
    New-Item -ItemType Directory -Path $destinationFolder | Out-Null
}

# Get the list of files from Fly.io
$files = fly sftp find /data/images
...
foreach ($file in $files) {
    ...
    # Extract the subdirectory structure from /data/images/profile
    $relativePath = $file -replace "^/data/images/", ""
