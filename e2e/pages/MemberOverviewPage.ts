...
export class MemberOverviewPage {
   ...
   async verifyUsersInList(names: string | string[], type: ListType) {
      if (typeof names === "string") {
         ...
         await expect(this.getList(type).getByRole("listitem")).toHaveCount(names.length);
   ...
   async addUser(username: string) {
      await this.getUserListItem(username, "loggedIn").getByRole("button", { name: "Legg til" }).click();
