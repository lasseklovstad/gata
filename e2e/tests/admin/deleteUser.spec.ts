...
test.describe("Delete user", () => {
  ...
  test("Should delete member that has added news", async ({
    adminPage,
    memberPage,
  }) => {
    ...
    await test.step("Check that news item is still there", async () => {
      ...
      await homePage.goto();
      await homePage.assertNewsHasContent(
        "Automatisk test",
        "Jeg kommer til å bli slette"
      );
  ...
  test("Should delete member has linked user", async ({ adminPage }) => {
    ...
    await memberOverviewPage.verifyUsersInList(
      [env.nonMemberUsername, env.memberUsername],
      "loggedIn"
    );
  });
});
...
test("Should not be able to delete your own user", async ({ adminPage }) => {
  ...
  await expect(
    adminPage.getByText("Du kan ikke slette deg selv!")
  ).toBeVisible();
});
...
const deleteUser = async (adminPage: Page) => {
  ...
  await memberOverviewPage.verifyUsersInList(
    [env.memberUsername, env.nonMemberUsername],
    "loggedIn"
  );
};
