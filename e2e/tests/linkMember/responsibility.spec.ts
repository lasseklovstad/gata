...
const editResponsibility = async (page: Page, name: string, content: string) => {
...
const removeResponsibilityFromMember = async (page: Page, responsibilityName: string) => {
...
const addResponsibilityToMember = async (page: Page, responsibilitName: string) => {
   const respPage = new ResponsibilityPage(page);
   await page.goto("/");
   await respPage.goto();
   await respPage.createNewResponsibility(responsibilitName, "Han er kul");
   await assignResponsibilityToMember(page, env.memberUsername, responsibilitName);
};

const assignResponsibilityToMember = async (page: Page, username: string, responsibilityName: string) => {
   const memberOverviewPage = new MemberOverviewPage(page);
   const memberPage = new MemberPage(page);
